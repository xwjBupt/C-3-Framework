...
class Trainer():
    def __init__(self, dataloader, cfg_data, pwd):
        ...
        if cfg.LR_CHANGER == 'step':
            ...
            self.scheduler = CosineAnnealingLR(self.optimizer, 2 * cfg.EPOCH_DIS, eta_min=5e-9, last_epoch=-1)
        elif cfg.LR_CHANGER == 'expotential':
            self.scheduler = ExponentialLR(self.optimizer, 0.95, last_epoch=-1)
        elif cfg.LR_CHANGER == 'rop':
            self.scheduler = ReduceLROnPlateau(self.optimizer, mode='min', verbose=True, eps=1e-10)
